...
type ResolveCorePricesInput = {
  inputToken?: string;
  outputToken?: string;
  inputTokenUsd?: number | string | null;
  outputTokenUsd?: number | string | null;
  stxUsd?: number | string | null;
  timeoutMs?: number;
  cacheFile?: string;
  userAgent?: string;
};

type ResolveCorePricesResult = {
  complete: boolean;
  btcUsd: NullableNumber;
  stxUsd: NullableNumber;
  inputTokenUsd: NullableNumber;
  outputTokenUsd: NullableNumber;
  fetchedAtUtc: string | null;
  sources: Record<string, string>;
  warnings: string[];
};

type ComputeProfitDiagnosticsInput = {
  amountIn: number | string;
  amountOut: number | string;
  minAmountOut: number | string;
  feeMicroStx?: number | string | null;
  feeStx?: number | string | null;
  inputTokenDecimals?: number | string | null;
  outputTokenDecimals?: number | string | null;
  inputTokenUsd?: number | string | null;
  outputTokenUsd?: number | string | null;
  stxUsd?: number | string | null;
};
...
function scaleAtomicToHuman(value: unknown, decimals: unknown): NullableNumber {
...
function ratioToBps(value: NullableNumber, base: NullableNumber): NullableNumber {
  if (!Number.isFinite(value) || !Number.isFinite(base) || base === 0) return null;
  return (value / base) * 10_000;
}

function percent(value: NullableNumber, base: NullableNumber): NullableNumber {
  if (!Number.isFinite(value) || !Number.isFinite(base) || base === 0) return null;
  return (value / base) * 100;
}

function deriveImpliedInputTokenUsd(inputTokenUsd: NullableNumber, input: Record<string, unknown> = {}): NullableNumber {
  if (Number.isFinite(inputTokenUsd)) return inputTokenUsd;
  if (!isStableUsdToken(String(input.outputToken || ''))) return inputTokenUsd;
...
function isBtcLikeToken(token?: string): boolean {
...
function readCache(cacheFile: string): any | null {
  try {
...
function writeCache(cacheFile: string, payload: Record<string, unknown>): void {
  try {
...
async function fetchCoinGecko(timeoutMs: number, userAgent: string): Promise<any> {
...
async function resolveCorePrices(input: ResolveCorePricesInput = {}): Promise<ResolveCorePricesResult> {
  const cacheFile =
    input.cacheFile ||
...
function computeProfitDiagnostics(input: ComputeProfitDiagnosticsInput) {
  ...
  const networkFeeStx = Number.isFinite(toFiniteNumber(input.feeStx))
    ? toFiniteNumber(input.feeStx)
